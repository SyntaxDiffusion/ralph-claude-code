...
REM Run ralph in Git Bash
echo Starting Ralph in Git Bash...
"%GITBASH%" -c "cd '%RALPH_DIR_UNIX%' && source ~/.bashrc 2>/dev/null; export PATH=\"$HOME/.local/bin:$PATH\"; if command -v ralph &>/dev/null; then ralph %ARGS%; else ./ralph_loop.sh %ARGS%; fi"
